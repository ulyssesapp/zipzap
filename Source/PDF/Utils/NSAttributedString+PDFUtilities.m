...
+ (NSAttributedString *)attributedStringWithNote:(RKPDFFootnote *)note enumerationString:(NSString *)enumerationString context:(RKPDFRenderingContext *)context
{
	...
	[noteString enumerateAttribute:RKParagraphStyleAttributeName inRange:NSMakeRange(0, noteString.length) options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired usingBlock:^(id ctParagraphStyle, NSRange range, BOOL *stop) {
		...
		[tabStops insertObject:[[RKTextTabWrapper alloc] initWithLocation:(context.document.footnoteAreaAnchorInset ?: 1) alignment:RKTextAlignmentToCTTextAlignment(context.document.footnoteAreaAnchorAlignment)] atIndex:0];
		[tabStops insertObject:[[RKTextTabWrapper alloc] initWithLocation:(context.document.footnoteAreaContentInset ?: 1) alignment:RKTextAlignmentToCTTextAlignment(RKTextAlignmentNatural)] atIndex:1];
